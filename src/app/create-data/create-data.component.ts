...
export class CreateDataComponent implements OnInit {
  ...
  selectedFiles: { [field: string]: File[] } = {};
...
async addData(imageUrls: any[]): Promise<void> {
  ...
  const docRef = await addDoc(collection(this.firestore, "product"), {
    heroHead: heroHead,
    heroContent: heroContent,
    heroImg: imageUrls.map((urlObj) => urlObj.heroImg),
    frameTittle: frameTittle,
    frameContent: frameContent,
    viewLastIntroTittle: viewLastIntroTittle,
    viewLastIntroContent: viewLastIntroContent,
  ...
  // }
  onFileSelected(event: any, field: string, index: number): void {
    const files: FileList = event.target.files;
    if (!this.selectedFiles[field]) {
      this.selectedFiles[field] = [];
    }
    this.selectedFiles[field][index] = files[0];
  }
  ...
  onSubmit(event: Event): void {
    ...
    const fields = Object.keys(this.selectedFiles);
